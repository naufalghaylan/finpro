...
export const updateUserAddressService = async (userId: number, addressId: number, data: Omit<Prisma.UserAddressUncheckedUpdateInput, 'userId'>) => {
  return await prisma.$transaction(async (tx) => {
    ...
    if (data.isPrimary) {
      // Unset primary for all other addresses
      await tx.userAddress.updateMany({
        where: { userId, isPrimary: true, id: { not: addressId }, deletedAt: null },
        data: { isPrimary: false }
      })
      ...
      const otherAddressCount = await tx.userAddress.count({
        where: { userId, id: { not: addressId }, deletedAt: null }
      })
...
export const deleteUserAddressService = async (userId: number, addressId: number) => {
  return await prisma.$transaction(async (tx) => {
    ...
    // Soft delete + lepas status primary agar tidak ada alamat aktif yang hilang acuannya
    await tx.userAddress.update({
      where: { id: addressId },
